...
var create bool
privateKey := &schema.Internal{Key: schema.InternalKey_LicenseValidationPrivateKey}
...
publicKey := &schema.Internal{Key: schema.InternalKey_LicenseValidationPublicKey}
...
MaxTries:         15,
Wait:             func(duration time.Duration) { time.Sleep(duration) },
Try: func() error {
req, err := http.NewRequest(http.MethodPost, proLogic.GetAccountsHost()+"/api/v1/license/validate", bytes.NewReader(requestBody))
if err != nil {
return err
}
